...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    {
      waterTankPositions.push_back({minX + (i + 1) * thickness, 0, 0});
    }
    ...
    for (int i = 0; i < numberOfLayers; i++)
    {
        ...
        // Task 5: Create 50 slices inside the water tank (use sName.str() for name)
        new G4PVPlacement(nullptr, waterTankPositions[i],
			  waterTankLog, sName.str(), worldLog, 0, i);
    }
...
void DetectorConstruction::ConstructSDandField()
{
    ...
    //G4MultiFunctionalDetector* waterTankDetector = new ........

    // Task 5.1: Create one primitive scorer for the dose and assign it to respective detectors

    //G4VPrimitiveScorer* waterTankScorer = new G4PSEnergyDeposit("........");
    //waterTankDetector->RegisterPrimitive(waterTankScorer);

    // Task 5.1 Assign multi-functional detectors to the logical volumes and register them to 
    //   the SDmanager

    //SetSensitiveDetector("waterTank", -------);
