...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
    ...
    for (int i = 0; i < numberOfLayers; i++)
    {
        ...
        new G4PVPlacement(nullptr, absorberPositions[i], absorberLog, aName.str(), worldLog, 0, i);
...
void DetectorConstruction::ConstructDemo(G4LogicalVolume *worldLog)
{
    ...
    G4VSolid *labelBox = new G4Box("label", thickness / 2, step / 2 * ayyData.size(), step / 2 * ayyData[0].size());
    ...
    for (auto line : ayyData)
    {
        ...
        for (auto chr : line)
        {
            if (chr == ' ')
            {
                G4ThreeVector pos{0, ((ayyData.size() - 1) / 2.0 - i) * step, (j - (line.size() - 1) / 2.0) * step};
...
void DetectorConstruction::ConstructSDandField()
{
    ...
    G4ThreeVector fieldValue = G4ThreeVector(0., 0.2 * tesla, 0.0);
