...
void EventAction::EndOfEventAction(const G4Event *event)
{
    ...
    // }
    // // Task 4c.2 : Similarly, retrieve fScintillatorId.How is the hit collection named ? As before, fScintillatorId is defined in EventAction.hh and initialized to - 1
    ...
    fAbsorberETId = sdm->GetCollectionID("absorberET/energy_time");
    // Task 4d.2: ...Retrieve fScintillatorETId
    fScintillatorETId = sdm->GetCollectionID("scintillatorET/energy_time");
    ...
    if (fScintillatorId >= 0)
    {
        ...
        auto hitMapS = dynamic_cast<G4THitsMap<G4double> *>(hcofEvent->GetHC(fScintillatorId));
    ...
    for (G4int collectionId : hitCollectionIds)
    {
        ...
        auto test = hcofEvent->GetHC(collectionId);

        // auto hitCollection = dynamic_cast<EnergyTimeHitsCollection *>(hcofEvent->GetHC(collectionId));
        // auto hitCollection = dynamic_cast<G4THitsMap<G4double> *>(hcofEvent->GetHC(collectionId));
        ...
        //         analysis->FillNtupleDColumn(1, hit->GetTime());
        //         analysis->FillNtupleDColumn(2, hit->GetPosition().getX());
        //         analysis->FillNtupleDColumn(3, hit->GetPosition().getY());
        //         analysis->FillNtupleDColumn(4, hit->GetPosition().getZ());
