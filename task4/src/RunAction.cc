                         ...
                         fTotalTrackLength("TotalTrackLength", 0.)
{
  ...
  //  CreateH1()
  analysisManager->CreateH1("eDep", "eDep in somewhere", 20, 50., 60.);

  // Task 4d.3: Create ntuple containing 5 double fields:
  //   EnergyDeposit, Time, X, Y & Z
  analysisManager->CreateNtuple("t4","Hits");
...
void RunAction::EndOfRunAction(const G4Run *run) // executes at the end of EACH run
{
  ...
  if (IsMaster())
  {
    ...
    if (fNGammas.GetValue())
      G4cout << " * Produced " << fNGammas.GetValue() / ((G4double)nofEvents) << " secondary gammas/event. Average energy: "
    ...
    if (fNElectrons.GetValue())
      G4cout << " * Produced " << fNElectrons.GetValue() / ((G4double)nofEvents) << " secondary electrons/event. Average energy: "
