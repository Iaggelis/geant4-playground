...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
  ...
  // Task 1b.1: Remove the "Krakow" label. It is nice but not very practical.
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  // G4VPrimitiveScorer* absorberScorer = new G4VPrimitiveScorer();
  ...
  // //------------------- Declare absorber as MultiFunctionalDetector---------------//

  // auto absorberDetector = new G4MultiFunctionalDetector("absorber");

  // auto primitive = new G4PSEnergyDeposit("energy");
  ...
  // //------------------- Declare scintillator as MultiFunctionalDetector---------------//

  // auto scintillatorDetector = new G4MultiFunctionalDetector("scintillator");

  // primitive = new G4PSEnergyDeposit("energy");
  ...
  EnergyTimeSD *scintillatorET = new EnergyTimeSD("scintillatorET");
  sdManager->AddNewDetector(absorberET);
  sdManager->AddNewDetector(scintillatorET);
}
